...
const server = createServer((req, res) => {
  ...
  if (req.method === "POST" && req.url === "/students") {
    ...
    return readBody(req, (err, newStudent) => {
      if (err) {
        return sendJson(res, 400, { message: "Invalid JSON" });
      }

      if (!newStudent.id || !newStudent.nombre || !newStudent.curso) {
        return sendJson(res, 400, { message: "Missing fields: id, nombre, curso required" });
      }

      const exists = students.some(st => st.id === newStudent.id);
      if (exists) return sendJson(res, 409, { message: "Student ID already exists" });
  ...
  if (req.method === "PUT" && req.url.startsWith("/students/")) {
    ...
    return readBody(req, (err, updatedData) => {
      if (err) return sendJson(res, 400, { message: "Invalid JSON" });

      // Actualizar solo los campos enviados
      if (updatedData.nombre) student.nombre = updatedData.nombre;
      if (updatedData.curso) student.curso = updatedData.curso;
